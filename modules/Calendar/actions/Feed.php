...
class Calendar_Feed_Action extends Vtiger_BasicAjax_Action {
	...
	protected function pullEvents($start, $end, &$result, $userid = false, $color = null, $textColor = 'white', $isGroupId = false, $conditions = '') {
		...
		//--Henry# para mostrar el nombre de cuenta
		$query = str_replace("vtiger_activity.subject", "vtiger_activity.subject , IFNULL( (
					SELECT vtiger_account.accountname FROM	vtiger_seactivityrel 
					INNER JOIN vtiger_crmentity ON vtiger_seactivityrel.crmid = vtiger_crmentity.crmid AND vtiger_crmentity.setype = 'Accounts'
					LEFT JOIN vtiger_account ON vtiger_seactivityrel.crmid = vtiger_account.accountid
		...
		while($record = $db->fetchByAssoc($queryResult)){
			...
			//--Henry# para obtener el nombre de la cuenta
			$item['accountname'] = '';
			if (!empty($record['accountname'])){
				$item['accountname'] = $record['accountname'];
			}
