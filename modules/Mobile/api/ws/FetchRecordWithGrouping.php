		...
		foreach($moduleFieldGroups as $blocklabel => $fieldgroups) {
			...
			foreach($fieldgroups as $fieldname => $fieldinfo) {
				...
				if(isset($resultRecord[$fieldname])) {
                    ...
                    /*
                    * Creantis-Henry: actualización para formatear los campos tipo MONEDA (currency)
                    */
                    $uitype_currency = array('71','72');
                    if ( in_array($field['uitype'], $uitype_currency) ) {
                    	$field['value'] = number_format(floatval($field['value']),2,".",",");
                    }
					$fields[] = $field;
				}
			}
