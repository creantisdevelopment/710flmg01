...
class Reports_Chart_Model extends Vtiger_Base_Model {
	...
	public static function getInstanceById($reportModel) {
		...
		if(!empty($data)) {
			...
			$self->setDataOrder($decodeData['dataOrder']); //--creantis
			$self->setDataTopN($decodeData['dataTopN']); //--creantis
	...
	//--Creantis- Henry: ordenamiento y top n
	function setDataOrder($dataOrder) {
		$this->dataOrder = $dataOrder;
	}
	function getDataOrder() {
		return $this->get('dataOrder');
	}
	function setDataTopN($dataTopN) {
		$this->dataTopN = $dataTopN;
	}
	function getDataTopN() {
		return $this->get('dataTopN');
	}
...
abstract class Base_Chart extends Vtiger_Base_Model{
	...
	function getQuery() {
		...
		if($this->isRecordCount()) {
			...
			$_order_by_cantidad = "RECORD_COUNT";
		}
		// Add other columns
		if($columns && is_array($columns)) {
			$columnLabels = array_merge($columnLabels, (array)$groupByColumns);
			$chartSQL .= implode(',', $columns);
			if ( empty($_order_by_cantidad))
				$_order_by_cantidad = trim(explode("AS", $columns[0])[1]);
		}

		//-- Creantis : Henry : ordenamiento por picklist
		$_arr = explode(",", $chartSQL);
		global $adb;
		$tbl_order = "";
		$field_order = "";
		$tbl_origen = "";
		foreach ($_arr as $value) {
			...
			if (count($x) > 1) {
				$tbl_origen = trim($x[0]);
				$field_order = trim(substr($x[1], 0, strrpos($x[1], "AS")));
				$table_picklist = "SHOW TABLES LIKE '%".$field_order."'";
				$rs_ = $adb->pquery($table_picklist, array());
				if ( $adb->num_rows($rs_) > 0 ) {
					$rs_pick = $adb->pquery("SELECT * FROM vtiger_field WHERE columnname = ? AND uitype IN (15,16,33)",array(trim($field_order)));
					if ( $adb->num_rows($rs_pick) == 1 ) {
						$tbl_order = trim('vtiger_'.$field_order);
					}
				}
			}
		}
		$order_by_picklist = '';
		if ( !empty($tbl_order) ) {
			$order_by_picklist = " `$tbl_order`.`sortorderid` ASC";
			$only_from = explode("WHERE", $sql[1]);
			$only_from[0] .= " LEFT JOIN `$tbl_order` ON `$tbl_order`.`$field_order` = `$tbl_origen`.`$field_order` ";
			$sql[1] = implode(" WHERE ", $only_from);
		}
		//-----------------------------------------------------
		...
		//-- Creantis - Henry: ordenamiento y top n
		$order_ = $this->getParent()->getDataOrder();
		if ( !empty($order_)) {
			$order_ = 'DESC';
		}
		// var_dump($columns);die();
		if ( !empty($order_) && strpos($chartSQL, "ORDER BY") === false ) {
			$chartSQL .= " ORDER BY ".$_order_by_cantidad." $order_";
		}
		
		if ( empty($order_) && !empty($order_by_picklist) ) {
			if ( strrpos($chartSQL, "ORDER BY") === false ) {
				$chartSQL .= " ORDER BY ";
			} else {
				$order_by_picklist = " , ".$order_by_picklist;
			}
			$chartSQL .= $order_by_picklist;
		}

		$limit_ = $this->getParent()->getDataTopN();
		if ( !empty($limit_) && $limit_ != 'all' && strpos($chartSQL, "LIMIT") === false ) {
			$chartSQL .= " LIMIT $limit_";
		}
	...
	function generateLink($field, $value) {
		...
		//-- Creantis : Henry ; parche para visualizar agrupación por semana y día
		...
		if(($dataFieldInfo[4] == 'D' || $dataFieldInfo[4] == 'DT') && !empty($dataFieldInfo[5])) {
			...
			if ( $dataFieldInfo[5] == 'WY' ||  $dataFieldInfo[5] == 'DY' ) {

				if ( $dataFieldInfo[5] == 'WY' ) {
					$arr_w = explode("/", $value);
					$year = trim($arr_w[1]);
					$week = trim( explode(" ", $arr_w[0])[1] );
					$week = ($week < 10)? '0'.$week : $week;
					$value = $year."W".$week;
					$date_start = date($date_format, strtotime($value));
					$date = new DateTime( $date_start );
					$date->modify("this week +6 days");
					$date_end = $date->format($date_format);
				} elseif ( $dataFieldInfo[5] == 'DY' ) {
					$date_start = date($date_format, strtotime($value)).' 00:00:00';
					$date_end = date($date_format, strtotime($value)). ' 23:59:59';
				}

				if(count($dataValue) > 1) {
					$comparator = 'bw';
					if($dataFieldInfo[4] == 'D') {
						$value = date($date_format, strtotime($date_start)).','.date($date_format, strtotime($date_end));
					} else {
						$value = date($date_format.' H:i:s' ,strtotime($date_start)).','.date($date_format ,strtotime($date_end)).' 23:59:59';
					}
				} else {
					$comparator = 'bw';
					$value = date($date_format, strtotime($date_start)).','.date($date_format, strtotime($date_end));
				...
				if(count($dataValue) > 1) {
					$comparator = 'bw';
					if($dataFieldInfo[4] == 'D') {
						$value = date($date_format, strtotime($value)).','.date($date_format, strtotime('last day of'.$value));
					} else {
						$value = date('Y-m-d H:i:s' ,strtotime($value)).','.date($date_format ,strtotime('last day of'.$value)).' 23:59:59';
					}
				} else {
					$comparator = 'bw';
					if($dataFieldInfo[4] == 'D') {
						$value = date($date_format, strtotime('first day of JANUARY '.$value)).','.date($date_format, strtotime('last day of DECEMBER '.$value));
					} else {
						$value = date($date_format.' H:i:s' ,strtotime('first day of JANUARY '.$value)).','.date($date_format ,strtotime('last day of DECEMBER '.$value)).' 23:59:59';
					}
				}
				...
				if ($fieldDataType == 'picklist') {
					...
					if($row[strtolower($legendField->get('reportlabel'))]) {
						...
						if ($groupByDataField[5] == 'M' || $groupByDataField[5] == 'Y' || $groupByDataField[5] == 'MY' 
								|| $groupByDataField[5] == 'WY' || $groupByDataField[5] == 'DY') { // Creantis - Henry: parche agrupación 
...
class VerticalbarChart extends Base_Chart {
	function generateData() {
		...
		$links = array();
		$j=-1;
		// var_dump($this->getParent()->getChartType());die();
		$chartType = $this->getParent()->getChartType();
		$dataOrder = $this->getParent()->getdataOrder();
		for($i = 0; $i < $rows; $i++) {
			//--Creantis - Henry: ordenar y top n 
			if ( ($chartType == 'verticalbarChart' && $dataOrder == 'ASC') || ($chartType == 'horizontalbarChart' && $dataOrder == 'DESC') ) {
				$r = ($rows-($i+1));
			} else {
				$r = $i;
			}
			$row = $db->query_result_rowdata($result, $r);
