...
class Reports_ChartSaveAjax_View extends Vtiger_IndexAjax_View {
	...
	public function process(Vtiger_Request $request) {
		...
		$dataTopN = $request->get("dataTopN");
		if (empty($dataTopN) || trim($dataTopN) == "" || $dataTopN == 0) {
			$dataTopN = 'all';
		} else {
			$dataTopN = intval($dataTopN);
		}
		// if (empty($dataOrder) || trim($dataOrder) == "") $dataOrder = 'DESC';
        ...
        // var_dump($reportChartModel->getDataOrder());die();
