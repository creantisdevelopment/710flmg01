            ...
            <optgroup label='{vtranslate($PRIMARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$PRIMARY_MODULE_NAME)}'>
                {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                    ...
                    {if $FIELD_INFO[4] eq 'D' or $FIELD_INFO[4] eq 'DT'}
                        ...
                        {* Creantis - Henry: nueva agrupación*}
                        <option value="{$FIELD_KEY}:WY">{vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)} ({vtranslate('LBL_WEEK', $PRIMARY_MODULE_NAME)})</option>
                        <option value="{$FIELD_KEY}:DY">{vtranslate($FIELD_LABEL, $PRIMARY_MODULE_NAME)} ({vtranslate('LBL_DAY', $PRIMARY_MODULE_NAME)})</option>
            ...
            <optgroup label='{vtranslate($SECONDARY_MODULE_NAME,$MODULE)}-{vtranslate($BLOCK_LABEL,$SECONDARY_MODULE_NAME)}'>
                {foreach key=FIELD_KEY item=FIELD_LABEL from=$BLOCK}
                    ...
                    {if $FIELD_INFO[4] eq 'D' or $FIELD_INFO[4] eq 'DT'}
                        ...
                        {* Creantis - Henry: nueva agrupación*}
                        <option value="{$FIELD_KEY}:WY">{vtranslate($FIELD_LABEL, $SECONDARY_MODULE_NAME)} ({vtranslate('LBL_WEEK', $SECONDARY_MODULE_NAME)})</option>
                        <option value="{$FIELD_KEY}:DY">{vtranslate($FIELD_LABEL, $SECONDARY_MODULE_NAME)} ({vtranslate('LBL_DAY', $SECONDARY_MODULE_NAME)})</option>
