...
************************************************************************************}
{strip}
{if $CHART_MODEL->getDataTopN() neq 'all'}
<style type="text/css">
.jqplot-point-label {
  font-size: 15px;
  padding: 1px 3px;
        ...
        <div class="reportsDetailHeader">
            ...
            <div class="filterElements contactAdd filterConditionsDiv{if !$REPORT_MODEL->isEditableBySharing()} hide{/if}">
                <form name='chartDetailForm' id='chartDetailForm' method="POST">
                    ...
                    {assign var=dataOrder value=$CHART_MODEL->getDataOrder()} 
                    {assign var=dataTopN value=$CHART_MODEL->getDataTopN()} 
                    ...
                    <div class="well filterConditionContainer">
                        <div>
                            ...
                            <div class='row'>
                                <span class="col-lg-4">
                                    <div><span>Top</span><span class="redColor"></span></div><br>
                                    <div>
                                        {literal}<script>function comprueba(_this){if(_this.value < 0 || _this.value == '.' || _this.value == ','){_this.value = ''}}</script>{/literal}
                                        <input type="number" name="dataTopN" id="dataTopN" class="inputElement validate[required]"  data-validation-engine="validate[required]" style='min-width:150px;width:150px;' value="{if $dataTopN eq 'all'}0{else}{$dataTopN}{/if}" onkeypress="comprueba(this);"  min="0" step="1" pattern="^[0-9]+">
                                    </div>
                                </span>
                                <span class="col-lg-2">&nbsp;</span>
                                <span class="col-lg-4">
                                    <div><span>Orden</span><span class="redColor"></span></div><br>
                                    <div>
                                        <select id='dataOrder' name='dataOrder' class="select2 validate[required]" data-validation-engine="validate[required]" style='min-width:300px;'>
                                            <option value="" >No Aplica</option>
                                            <option value="DESC" {if $dataOrder eq "DESC"} selected {/if}>Descendente</option>
                                            <option value="ASC" {if $dataOrder eq "ASC"} selected {/if}>Ascendente</option>
                                        </select></div>
                                </span>
                            </div>
                            <br>
