	...
	<div>
		...
		<div>
			<div class='pull-left'>
				...
				{* Henry - Para mostrar el nombre de la cuenta *}
				<a href="{$ACTIVITY->getDetailViewUrl()}">{$ACTIVITY->get('subject')} 
					{if $ACTIVITY->get('accountname') != "" } (Cuenta: {$ACTIVITY->get('accountname')} ) {/if}
				</a>
				{if $PARENT_ID} {vtranslate('LBL_FOR')} {$ACTIVITY->getDisplayValue('parent_id')}{else if $CONTACT_ID} {vtranslate('LBL_FOR')} {$ACTIVITY->getDisplayValue('contact_id')}{/if}
			</div>
