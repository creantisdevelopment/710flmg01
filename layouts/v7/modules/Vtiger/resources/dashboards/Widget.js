...
Vtiger.Class('Vtiger_Widget_Js',{
	...
	container : false,
	plotContainer : false,
	//--Henry# flags para controlar la nicializacion de los widgets
	objVal_v : {
		Pipelined_Amount_v: 'Pipelined Amount',
		Potentials_by_Stage_v: 'Potentials by Stage',
		Funnel_v: 'Funnel',
		Funnel_Amount_v: 'Funnel Amount',
		Total_Revenue_v: 'Total Revenue'
	},
	objVal : {
		Pipelined_Amount: 0,
		Potentials_by_Stage: 0,
		Funnel: 0,
		Funnel_Amount: 0,
		Total_Revenue: 0
	},
	...
	init : function (container) {
		//--Henry# para evitar que se acumulen los registros de OnClick
		var widget_name = jQuery(".widgetChartContainer", container);
		var widget_name = jQuery(widget_name).data("type");
		widget_name = (widget_name!=undefined)? widget_name.trim() : '';
		switch(widget_name) {
			case this.objVal_v.Pipelined_Amount_v: if(this.objVal.Pipelined_Amount==0) this.init_(container); this.objVal.Pipelined_Amount++; break;
			case this.objVal_v.Potentials_by_Stage_v: if(this.objVal.Potentials_by_Stage==0) this.init_(container); this.objVal.Potentials_by_Stage++; break;
			case this.objVal_v.Funnel_v: if(this.objVal.Funnel==0) this.init_(container); this.objVal.Funnel++; break;
			case this.objVal_v.Funnel_Amount_v: if(this.objVal.Funnel_Amount==0) this.init_(container); this.objVal.Funnel_Amount++; break;
			case this.objVal_v.Total_Revenue_v: if(this.objVal.Total_Revenue==0) this.init_(container); this.objVal.Total_Revenue++; break;
			case this.objVal_v.Total_Revenue_v: if(this.objVal.Total_Revenue==0) this.init_(container); this.objVal.Total_Revenue++; break;
			default: 
				if ( this.objVal[widget_name] == undefined || this.objVal[widget_name] == null || this.objVal[widget_name] == 0) {
					this.objVal_v[widget_name + "_v"] = widget_name;
					this.objVal[widget_name] = 0;
					this.init_(container); 
				}else if (widget_name == undefined || widget_name == '' || widget_name == null){
					this.init_(container); 
				}
				this.objVal[widget_name]++;
				break;
		}
		// console.log("count", widget_name, this.objVal);
		// console.log("s", this.objVal_v.Pipelined_Amount_v, this.objVal.Pipelined_Amount);
	},
	init_ : function(container){
	...
	postRefreshWidget : function() {
		// console.log("postRefreshWidget this.isEmptyData()", this.isEmptyData());
		if(!this.isEmptyData()) {
			this.loadChart();
            this.postInitializeCalls();
		}else{
...
Vtiger_Widget_Js('Vtiger_Funnel_Widget_Js',{},{
    ...
    postInitializeCalls: function() {
        // console.log("postInitializeCalls");
        var thisInstance = this;
	...
	loadChart : function() {
		// console.log("loadChart", data, this.generateLinks());
		var container = this.getContainer();
