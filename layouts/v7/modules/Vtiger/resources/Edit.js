	...
	registerEventForPicklistDependencySetup : function(container){
		...
		sourcePickListElements.on('change',function(e){
			...
			jQuery.each(picklistmap,function(targetPickListName,targetPickListValues){
				...
				//-- Creantis: Henry: parche para no resetear la lista dependiente si es que el valor existe, guarda el últivo valor seleccionado
                var lastValue = targetPickList.val();
				...
				//-- Creantis: Henry: parche para no resetear la lista dependiente si es que el valor existe
                // targetPickList.html(targetOptions).val(targetPickListSelectedValue).trigger("change"); // vtiger
                targetPickList.html(targetOptions); // crreantis
                if ( targetPickList.find("option[value='"+lastValue+"']").length != 0) {
                    targetPickList.val(lastValue).trigger("change");
                } else {
                    targetPickList.val("").trigger("change");
                }
