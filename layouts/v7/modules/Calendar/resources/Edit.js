...
Vtiger_Edit_Js("Calendar_Edit_Js",{
	...
	registerRelatedContactSpecificEvents : function(form) {
		...
		this.getRelatedContactElement(form).select2({
			...
			//--Henry# Para que pase datos de multiselect a formulario completo
			console.log("multiselect", data.added);
			if ( data.added != undefined) {
				var t = data.added;
				var data = {'data' : [{'id':t.id, 'name':t.label, 'text': t.label}]};
				thisInstance.addNewContactToRelatedList(data,form);
			}
		});
