...
class Vtiger_Module extends Vtiger_ModuleBasic {
	 ...
	 * @param boolean Flag Para crear la relacion de vuelta (Módulos nuevos) //-Creantis: Henry - 2017-12-18
	 *
	 * @internal Creates table vtiger_crmentityrel if it does not exists
	 */
	function setRelatedList($moduleInstance, $label = '', $actions = false, $function_name = 'get_related_list', $fieldId = null, $flg_return_relation = false) {
		global $adb;
		...
		* Creantis: Henry
		* 2017-12-18
		* Crea la relación de vuelta con el módulo nuevo
		* Agregar $flg_return_relation como parametros de entrada, este metodo es llamado desde PackageImport.php
		*/
		if ( $flg_return_relation ) {
			$relation_id = $this->__getRelatedListUniqueId();
			$sequence = $this->__getNextRelatedListSequence();
			$label = vtlib_getModuleNameById($this->id);
			$adb->pquery("INSERT INTO vtiger_relatedlists(relation_id,tabid,related_tabid,name,sequence,label,presence,actions) VALUES(?,?,?,?,?,?,?,?)",
			Array($relation_id,$moduleInstance->id,$this->id,$function_name,$sequence,$label,$presence,''));
		}
