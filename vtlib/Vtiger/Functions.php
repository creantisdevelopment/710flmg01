...
class Vtiger_Functions {
	...
	static function getModuleFieldInfos($mixed) {
		...
		if ($module) {
			//-- Creantis: Henry: 2018-11-09: para que la vista relacionada pueda mostrar campos CF tanto de eventos como de tareas
			$tabid = self::getModuleId($module);
			if ($module == 'Calendar') {
	    		$tabid = array('9','16');
	    	}
			$result = $adb->pquery("SELECT * FROM vtiger_field WHERE tabid in (" . generateQuestionMarks($tabid) . ")", array($tabid));
